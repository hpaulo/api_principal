    ...
    public class WebpagesUsersInRolesController : ApiController
    {
        ...
        public HttpResponseMessage Get(string token, int colecao = 0, int campo = 0, int orderBy = 0, int pageSize = 0, int pageNumber = 0)
        {
            ...
            try
            {
                ...
                {
                    Retorno dados = GatewayWebpagesUsersInRoles.Get(token, colecao, campo, orderBy, pageSize, pageNumber, queryString);
        ...
        public HttpResponseMessage Post(string token, [FromBody]webpages_UsersInRoles param)
        {
            ...
            try
            {
                ...
                if (Permissoes.Autenticado(token))
                {
                    Int32 dados = GatewayWebpagesUsersInRoles.Add(token, param);
                    log.codResposta = (int)HttpStatusCode.OK;
                    Bibliotecas.LogAcaoUsuario.Save(log);
