    ...
    public class TbContaCorrenteController : ApiController
    {
        ...
        public HttpResponseMessage Get(string token, int colecao = 0, int campo = 0, int orderBy = 0, int pageSize = 0, int pageNumber = 0)
        {
            ...
            using (painel_taxservices_dbContext _db = new painel_taxservices_dbContext())
            {
                ...
                try
                {
                    ...
                    {
                        Retorno dados = GatewayTbContaCorrente.Get(token, colecao, campo, orderBy, pageSize, pageNumber, queryString, _db);
        ...
        public HttpResponseMessage Post(string token, [FromBody]tbContaCorrente param)
        {
            ...
            using (painel_taxservices_dbContext _db = new painel_taxservices_dbContext())
            {
                ...
                try
                {
                    ...
                    if (Permissoes.Autenticado(token, _db))
                    {
                        Int32 dados = GatewayTbContaCorrente.Add(token, param, _db);
                        log.codResposta = (int)HttpStatusCode.OK;
                        Bibliotecas.LogAcaoUsuario.Save(log, _db);
        ...
        public HttpResponseMessage Delete(string token, Int32 cdContaCorrente)
        {
            ...
            using (painel_taxservices_dbContext _db = new painel_taxservices_dbContext())
            {
                ...
                try
                {
                    log = Bibliotecas.LogAcaoUsuario.New(token, JsonConvert.SerializeObject("cdContaCorrente : " + cdContaCorrente), "Delete", _db);

                    HttpResponseMessage retorno = new HttpResponseMessage();
                    if (Permissoes.Autenticado(token, _db))
                    {
                        GatewayTbContaCorrente.Delete(token, cdContaCorrente, _db);
                        log.codResposta = (int)HttpStatusCode.OK;
                        Bibliotecas.LogAcaoUsuario.Save(log, _db);
                        return Request.CreateResponse(HttpStatusCode.OK);
                    }
