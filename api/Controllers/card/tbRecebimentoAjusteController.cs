...
{
    public class TbRecebimentoAjusteController : ApiController
    {

        // GET /tbRecebimentoAjuste/token/colecao/campo/orderBy/pageSize/pageNumber?CAMPO1=VALOR&CAMPO2=VALOR
        public HttpResponseMessage Get(string token, int colecao = 0, int campo = 0, int orderBy = 0, int pageSize = 0, int pageNumber = 0)
        {
            ...
            try
            {
                ...
                {
                    Retorno dados = GatewayTbRecebimentoAjuste.Get(token, colecao, campo, orderBy, pageSize, pageNumber, queryString, _db);
        ...
        public HttpResponseMessage Post(string token, [FromBody]tbRecebimentoAjuste param)
        {
            ...
            try
            {
                ...
                {
                    Int32 dados = GatewayTbRecebimentoAjuste.Add(token, param, _db);
                    log.codResposta = (int)HttpStatusCode.OK;
                    Bibliotecas.LogAcaoUsuario.Save(log, _db);
        ...
        public HttpResponseMessage Delete(string token, Int32 idRecebimentoAjuste)
        {
            ...
            try
            {
                log = Bibliotecas.LogAcaoUsuario.New(token, JsonConvert.SerializeObject("idRecebimentoAjuste : " + idRecebimentoAjuste), "Delete", _db);

                HttpResponseMessage retorno = new HttpResponseMessage();
                if (Permissoes.Autenticado(token, _db) && Permissoes.GetRoleLevel(token, _db) <= 1)
                {
                    GatewayTbRecebimentoAjuste.Delete(token, idRecebimentoAjuste, _db);
                    log.codResposta = (int)HttpStatusCode.OK;
                    Bibliotecas.LogAcaoUsuario.Save(log, _db);
                    return Request.CreateResponse(HttpStatusCode.OK);
                }
