...
{
    public class TbAdquirenteController : ApiController
    {
        ...
        public HttpResponseMessage Post(string token, [FromBody]tbAdquirente param)
        {
            ...
            try
            {
                ...
                if (Permissoes.Autenticado(token, _db))
                {
                    Int32 dados = GatewayTbAdquirente.Add(token, param, _db);
                    log.codResposta = (int)HttpStatusCode.OK;
                    Bibliotecas.LogAcaoUsuario.Save(log, _db);
        ...
        public HttpResponseMessage Put(string token, [FromBody]tbAdquirente param)
        {
            ...
            try
            {
                ...
                if (Permissoes.Autenticado(token, _db))
                {
                    GatewayTbAdquirente.Update(token, param, _db);
                    log.codResposta = (int)HttpStatusCode.OK;
                    Bibliotecas.LogAcaoUsuario.Save(log, _db);
        ...
        public HttpResponseMessage Delete(string token, Int32 cdAdquirente)
        {
            ...
            try
            {
                log = Bibliotecas.LogAcaoUsuario.New(token, JsonConvert.SerializeObject("cdAdquirente : " + cdAdquirente), "Delete", _db);

                HttpResponseMessage retorno = new HttpResponseMessage();
                if (Permissoes.Autenticado(token, _db))
                {
                    GatewayTbAdquirente.Delete(token, cdAdquirente, _db);
                    log.codResposta = (int)HttpStatusCode.OK;
                    Bibliotecas.LogAcaoUsuario.Save(log, _db);
                    return Request.CreateResponse(HttpStatusCode.OK);
                }
