    ...
    public class TbSituacaoRedeTefController : ApiController
    {
        ...
        public HttpResponseMessage Get(string token, int colecao = 0, int campo = 0, int orderBy = 0, int pageSize = 0, int pageNumber = 0)
        {
            ...
            try
            {
                ...
                {
                    Retorno dados = GatewayTbSituacaoRedeTef.Get(token, colecao, campo, orderBy, pageSize, pageNumber, queryString);
        ...
        public HttpResponseMessage Post(string token, [FromBody]tbSituacaoRedeTef param)
        {
            ...
            try
            {
                ...
                if (Permissoes.Autenticado(token))
                {
                    short dados = GatewayTbSituacaoRedeTef.Add(token, param);
                    log.codResposta = (int)HttpStatusCode.OK;
                    Bibliotecas.LogAcaoUsuario.Save(log);
        ...
        public HttpResponseMessage Delete(string token, short cdSituacaoRedeTef)
        {
            ...
            try
            {
                log = Bibliotecas.LogAcaoUsuario.New(token, JsonConvert.SerializeObject("cdSituacaoRedeTef : " + cdSituacaoRedeTef), "Delete");
