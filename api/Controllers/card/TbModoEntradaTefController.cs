    ...
    public class TbModoEntradaTefController : ApiController
    {
        ...
        public HttpResponseMessage Get(string token, int colecao = 0, int campo = 0, int orderBy = 0, int pageSize = 0, int pageNumber = 0)
        {
            ...
            try
            {
                ...
                {
                    Retorno dados = GatewayTbModoEntradaTef.Get(token, colecao, campo, orderBy, pageSize, pageNumber, queryString);
        ...
        public HttpResponseMessage Post(string token, [FromBody]tbModoEntradaTef param)
        {
            ...
            try
            {
                ...
                if (Permissoes.Autenticado(token))
                {
                    short dados = GatewayTbModoEntradaTef.Add(token, param);
                    log.codResposta = (int)HttpStatusCode.OK;
                    Bibliotecas.LogAcaoUsuario.Save(log);
        ...
        public HttpResponseMessage Delete(string token, short cdModoEntradaTef)
        {
            ...
            try
            {
                log = Bibliotecas.LogAcaoUsuario.New(token, JsonConvert.SerializeObject("cdModoEntradaTef : " + cdModoEntradaTef), "Delete");
