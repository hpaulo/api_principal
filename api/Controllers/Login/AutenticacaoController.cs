                        ...
                        if (colecao == 1)
                        {
                            ...
                            if (verify != null)
                            {
                                ...
                                .Select(r => new
                                {
                                    ...
                                    RolePrincipal = r.RolePrincipal,
                                    ControllerPrincipal = r.webpages_Roles.webpages_Permissions.Where(p => p.fl_principal == true).Where(p => p.webpages_Methods.ds_method.ToUpper().Equals("LEITURA")).Select(p => p.webpages_Methods.webpages_Controllers.id_controller).FirstOrDefault(), // .FirstOrDefault().webpages_Methods.webpages_Controllers.id_controller,
                                    Controllers = _db.webpages_Permissions.Where(p => p.id_roles == r.RoleId)
                                                                          .Where(p => p.webpages_Methods.ds_method.ToUpper().Equals("LEITURA"))
                                                                          .Select(p => new { id_controller = p.webpages_Methods.id_controller, 
                                                                                             ds_controller = p.webpages_Methods.webpages_Controllers.ds_controller,
                                                                                             id_subController = p.webpages_Methods.webpages_Controllers.id_subController
                                                                                           }
                                                                                 )
                                                                          .ToList<dynamic>(),
                                    FiltroEmpresa = _db.webpages_Permissions.Where(p => p.id_roles == r.RoleId).Where(p => p.webpages_Methods.ds_method.ToUpper().Equals("FILTRO EMPRESA")).Select(p => new { id_controller = p.webpages_Methods.id_controller }).ToList<dynamic>().Count > 0,
                                ...
                                if (!Permissoes.isAtosRole(token))
                                {
                                    ...
                                    cardservices = _db.Database.SqlQuery<webpages_Controllers>("WITH CTRL AS (SELECT ds_controller, id_controller, id_subController, nm_controller, fl_menu FROM dbo.webpages_Controllers WHERE ds_controller = '[Mobile] Card Services' AND id_subController IS NULL AND id_controller > 50 UNION ALL SELECT c.ds_controller, c.id_controller, c.id_subController, c.nm_controller, c.fl_menu FROM dbo.webpages_Controllers c INNER JOIN CTRL s ON c.id_subController = s.id_controller) SELECT * FROM CTRL").ToList<webpages_Controllers>();
                                    proinfo = _db.Database.SqlQuery<webpages_Controllers>("WITH CTRL AS (SELECT ds_controller, id_controller, id_subController, nm_controller, fl_menu FROM dbo.webpages_Controllers WHERE ds_controller = '[Mobile] Pro Info' AND id_subController IS NULL AND id_controller > 50 UNION ALL SELECT c.ds_controller, c.id_controller, c.id_subController, c.nm_controller, c.fl_menu FROM dbo.webpages_Controllers c INNER JOIN CTRL s ON c.id_subController = s.id_controller) SELECT * FROM CTRL").ToList<webpages_Controllers>();
                                    taxservices = _db.Database.SqlQuery<webpages_Controllers>("WITH CTRL AS (SELECT ds_controller, id_controller, id_subController, nm_controller, fl_menu FROM dbo.webpages_Controllers WHERE ds_controller = '[Mobile] Tax Services' AND id_subController IS NULL AND id_controller > 50 UNION ALL SELECT c.ds_controller, c.id_controller, c.id_subController, c.nm_controller, c.fl_menu FROM dbo.webpages_Controllers c INNER JOIN CTRL s ON c.id_subController = s.id_controller) SELECT * FROM CTRL").ToList<webpages_Controllers>();
                                }
                                ...
                                foreach (var item in permissoes)
                                {
                                    ...
                                    foreach (var subItem in item.Controllers)
                                    {
                                        ...
                                        if (Mobile && mobileControllers.Any(m => m.id_controller == subItem.id_controller))
                                        {
                                            if (cardservices.Any(c => c.id_controller == subItem.id_controller))
                                            {
                                                if (subItem.id_subController == null || fl_cardservices) list.Add(subItem.id_controller); // só add se for um controller sem pai ou tiver acesso ao card services
                                            }
                                            // É de Pro info ?
                                            else if (proinfo.Any(c => c.id_controller == subItem.id_controller))
                                            {
                                                if (subItem.id_subController == null || fl_proinfo) list.Add(subItem.id_controller); // só add se for um controller sem pai ou tiver acesso ao pro info
                                            }
                                            // É de Tax Service ?
                                            else if (taxservices.Any(c => c.id_controller == subItem.id_controller))
                                            {
                                                if (subItem.id_subController == null || fl_taxservices) list.Add(subItem.id_controller); // só add se for um controller sem pai ou tiver acesso ao tax services
                                            }
                                            else list.Add(subItem.id_controller);
                                        }
                                    }
            ...
            try
            {
                if (ModelState.IsValid && WebSecurity.Login(data.usuario, data.senha, persistCookie: false))
                {
                    ...
                    using (var _db = new painel_taxservices_dbContext())
                    {
                        ...
                        return new Models.Object.Autenticado { nome = usuario.nm_pessoa, usuario = usuario.ds_login, token = token, controllers = controllers, id_grupo = (usuario.grupo_empresa == null ? -1 : (Int32)usuario.grupo_empresa.id_grupo), };
                    }
